 ...
 * frame.
 *------------------------------------------------------------------------------ */
void Driver::check_neigh(const int id, DumpAtom *one, DumpAtom *now, std::list<int> * ids, int *cid)
{
  int ni  = now->neilist[0][id];
  int nip = one->neilist[0][id];
  for (int jj = 1; jj <= ni; ++jj){
    int jd = now->neilist[jj][id];
    if (cid[jd] > 0) continue;

    int hit = 0;
    for (int kk = 1; kk <= nip; ++kk) if (one->neilist[kk][id] == jd) {++hit; break;}
    if (hit){
      cid[jd] = cid[id];
      ids->push_back(jd);

      check_neigh(jd, one, now, ids, cid);
    }
  }
return;
}
